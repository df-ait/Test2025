...
auto Model<T>::forward(Matrix<T>& introduc_M)
{
    ...
    Matrix index_2 = bias_1.matrix_add(index_1);
    //relu函数，得到长宽为1 * 500的临时矩阵
    Matrix index_3 = Relu(index_2);
    //再和weight2做矩阵乘法，得到1 * 10的临时矩阵
    Matrix index_4 = index_3.matrix_multipy(weight_2);
    //再和bias2做矩阵加法，得到1 * 10的矩阵
    Matrix index_5 = bias_2.matrix_add(index_4);
...
Matrix<T> Model<T>::Relu(Matrix<T>& A)
{
    for(auto i = 0 ; i< A.line ; i++){
        for(auto j = 0 ; j< A.column ; j++){
            A.matrix[i][j] = std::fmax(0.0 , A.matrix[i][j]);
        }
    }
    return A;
}
